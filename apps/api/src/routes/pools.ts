...
const poolIdSchema = z.object({
  params: z.object({
    poolId: z.string().min(1).max(66).refine(
      (val) => /^[0-9a-f-]{36}$/i.test(val) || /^0x[a-fA-F0-9]{40}$/.test(val),
...
router.post(
  ...
  async (req, res, next) => {
    try {
      // Security: Verify the authenticated user owns this pool
      const requestedAddress = req.params.address.toLowerCase()
      const userAddress = req.user?.address?.toLowerCase()

      if (!userAddress || requestedAddress !== userAddress) {
        res.status(403).json({
          error: 'Forbidden',
          message: 'You can only refresh your own pool stats',
        })
        return
      }
