...
import type { User, Pool, Deposit, EventLog, PoolType, DepositType, TransactionStatus } from '@prisma/client'
...
describe('AnalyticsService', () => {
  ...
  describe('getGlobalStats', () => {
    ...
    it('should only count active pools', async () => {
      ...
      expect(prisma.pool.count).toHaveBeenCalledWith({ where: { status: 'ACTIVE' } })
      expect(prisma.pool.findMany).toHaveBeenCalledWith({ where: { status: 'ACTIVE' } })
    })
