...
import type { Pool, PoolAnalytics, Deposit, User, PoolType, DepositType, TransactionStatus } from '@prisma/client'
...
describe('PoolsService', () => {
  ...
  describe('getPoolById', () => {
    ...
    it('should throw AppError 404 when pool does not exist', async () => {
      ...
      await expect(poolsService.getPoolById('nonexistent')).rejects.toThrow(AppError)
      await expect(poolsService.getPoolById('nonexistent')).rejects.toThrow('Pool not found')
    })
  })
  ...
  describe('getPoolByAddress', () => {
    ...
    it('should throw AppError 404 when pool not found', async () => {
      ...
      await expect(poolsService.getPoolByAddress('0xnonexistent')).rejects.toThrow(AppError)
    })
  })
  ...
  describe('getPoolUsers', () => {
    it('should return users with positive balances sorted by balance', async () => {
      const mockDeposits: (Deposit & { user: Pick<User, 'address' | 'ensName' | 'avatar'> })[] = [
        {
          id: '1',
          userId: 'user1',
          userAddress: '0xuser1',
  ...
  describe('createAnalyticsSnapshot', () => {
    ...
    it('should throw AppError 404 when pool does not exist', async () => {
      ...
      await expect(poolsService.createAnalyticsSnapshot('nonexistent')).rejects.toThrow(AppError)
      await expect(poolsService.createAnalyticsSnapshot('nonexistent')).rejects.toThrow('Pool not found')
    })
