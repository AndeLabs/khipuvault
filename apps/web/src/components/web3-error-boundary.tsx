 ...
 */
function Web3ErrorFallback({ error, errorInfo, resetError }: ErrorFallbackProps) {
      ...
      <Card className="max-w-2xl w-full border-border">
        <CardHeader>
          <div className="flex items-center gap-3 mb-2">
            <div className={`h-12 w-12 rounded-full flex items-center justify-center ${getErrorColor()}`}>
              {getErrorIcon()}
            </div>
            <div>
        ...
        <CardContent className="space-y-4">
          {/* Error Message */}
          <Alert variant={parsedError.type === Web3ErrorType.TRANSACTION_REJECTED ? 'default' : 'destructive'}>
            <AlertDescription>
              {parsedError.message}
            </AlertDescription>
          </Alert>
        ...
        <CardFooter className="flex gap-3 flex-wrap">
          ...
          <Button
            onClick={resetError}
            variant={parsedError.type === Web3ErrorType.WALLET_DISCONNECTED && !isConnected ? 'outline' : 'default'}
            className="gap-2"
            size="lg"
          >
            <RefreshCw className="h-4 w-4" />
            {parsedError.type === Web3ErrorType.TRANSACTION_REJECTED ? 'Reintentar Transaccion' : 'Reintentar'}
          </Button>
