...
export function MyPoolsV3() {
  ...
  return (
    <>
      ...
      <Dialog
        open={selectedAction?.type === "leave"}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Recibirás tu contribución de BTC más los yields generados. Esta
...
function MyPoolCard({
  ...
  const { memberInfo, isLoading: isLoadingMember } = useMemberInfo(
    poolId,
    address,
  );
  ...
  const memberSharePercentage =
    totalShares > 0n
      ? (Number(memberInfo.shares) / Number(totalShares)) * 100
      : 0;

  const estimatedYield =
    totalShares > 0n && poolInfo.totalYieldGenerated > 0n
  ...
  const daysSinceMembership = Math.floor(
    (Date.now() / 1000 - memberInfo.joinedAt) / 86400,
  );
    ...
    >
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 rounded-lg bg-primary/10 border border-primary/30">
            ...
            <p className="text-sm font-bold text-foreground">
              {parseFloat(formatEther(memberInfo.btcContributed)).toFixed(4)}{" "}
              BTC
            </p>
          ...
          <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/30">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {parseFloat(formatEther(memberInfo.yieldClaimed)).toFixed(4)}{" "}
              reclamado
            </p>
          </div>
        </div>
        ...
        {hasYieldToClaim && (
          <Alert className="bg-green-500/10 border-green-500/30 p-3">
            ...
            <AlertDescription className="text-green-200 text-xs">
              <strong className="text-green-400">Yields disponibles!</strong>{" "}
              Puedes reclamarlos sin retirar tu principal.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground pt-2 border-t border-muted">
          ...
          <span>
            Al salir recibirás tu BTC + yields. Puedes reclamar yields sin salir
            del pool.
          </span>
        </div>
      </CardContent>
