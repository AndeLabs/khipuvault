/**
 * @fileoverview Enhanced Position Display with Smart APR Calculation
 * Shows pool APR when user APR not available yet (<24h)
 */
...
import { TrendingUp, Wallet, Clock, Zap, Users, Gift, Sparkles, Info } from 'lucide-react'
import { useState } from 'react'
import { formatUnits } from 'viem'
import { useReadContract } from 'wagmi'
import { type Address } from 'viem'

const YIELD_AGGREGATOR_ADDRESS = '0x3D28A5eF59Cf3ab8E2E11c0A8031373D46370BE6' as Address
...
export function PositionEnhanced() {
  const { 
    userInfo, 
    hasActiveDeposit, 
    poolTVL,
  ...
  // Read pool APR from YieldAggregator
  const { data: poolAPR } = useReadContract({
    address: YIELD_AGGREGATOR_ADDRESS,
    abi: AGGREGATOR_ABI,
    functionName: 'getAverageApr',
  })
  ...
  // Smart APR calculation:
  // - Use user APR if available (daysActive > 0)
  // - Otherwise use pool APR
  const userAPR = userInfo?.estimatedAPR || 0n
  const effectiveAPR = daysActive > 0 ? userAPR : (poolAPR || 0n)
  const aprSource = daysActive > 0 ? 'personal' : 'pool'
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="p-4 rounded-lg bg-card/50 border border-blue-500/30">
              ...
              <p className="text-2xl font-bold text-blue-500">
                {formatAPR(effectiveAPR)}
              </p>
              <div className="flex items-center gap-1 mt-1">
                <p className="text-xs text-muted-foreground">
                  {aprSource === 'personal' ? 'Tu APR real' : 'APR del pool'}
                </p>
                {aprSource === 'pool' && (
                  <div className="group relative">
                    <Info className="h-3 w-3 text-muted-foreground cursor-help" />
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-48 p-2 bg-card border border-primary/30 rounded-lg text-xs text-muted-foreground z-10">
                      Tu APR personal se calculará después de 24 horas. Mientras tanto, se muestra el APR promedio del pool.
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="p-4 rounded-lg bg-card/50 border border-yellow-500/30">
              ...
              <p className="text-2xl font-bold text-yellow-500">
                {daysActive > 0 ? `${daysActive}d` : '<1d'}
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                {daysActive === 0 ? 'Menos de 24 horas' : 'Días activo'}
              </p>
            </div>
          </div>

          {/* Info Banner for <24h users */}
          {daysActive === 0 && (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-blue-200">
                    Tu APR personalizado se calculará después de 24 horas de actividad. 
                    Actualmente se muestra el APR promedio del pool ({formatAPR(poolAPR || 0n)}).
