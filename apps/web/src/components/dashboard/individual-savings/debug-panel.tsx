...
export function DebugPanel() {
  ...
  const { userInfo, poolTVL, isLoading: poolLoading } = useIndividualPoolV3();

  const { musdBalance, balanceFormatted, allowance } = useMusdApproval();
    ...
    <Card className="bg-card border border-yellow-500/50">
      ...
      <CardContent className="space-y-4 text-xs font-mono">
        <div>
          ...
          <p className="text-white">
            Chain ID: {chainId}{" "}
            {chainId === 31611 ? "✅ Mezo Testnet" : "❌ Wrong Network"}
          </p>
          <p className="text-white">
            Native BTC:{" "}
            {nativeBalance ? Number(nativeBalance.formatted).toFixed(6) : "0"}{" "}
            BTC
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-white text-xs break-all">
            MUSD: {MEZO_TESTNET_ADDRESSES.musd}
          </p>
          <p className="text-white text-xs break-all">
            Pool: {MEZO_TESTNET_ADDRESSES.individualPool}
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-white">
            Pool Loading: {poolLoading ? "⏳ Yes" : "✅ No"}
          </p>
        </div>
        ...
        <div className="pt-2 border-t border-yellow-500/30">
          ...
          {musdBalance && Number(musdBalance) > 0 ? (
            <p className="text-green-500 text-xs">
              ✅ Tienes MUSD en tu wallet. Puedes depositar!
            </p>
          ) : (
            <div className="space-y-2">
              <p className="text-red-400 text-xs">
                ❌ No tienes MUSD en tu wallet
              </p>
              <div className="bg-blue-500/10 border border-blue-500/30 rounded p-2">
                <p className="text-blue-400 text-xs font-semibold mb-1">
                  Si tienes MUSD en Stability Pool de Mezo:
                </p>
              ...
              <div className="bg-green-500/10 border border-green-500/30 rounded p-2">
                <p className="text-green-400 text-xs font-semibold mb-1">
                  Si quieres obtener MUSD con tu BTC (
                  {nativeBalance
