...
const CountdownTimer = ({ endTime }: CountdownTimerProps) => {
  ...
  return (
    <div className="flex items-center gap-2">
      <span role="img" aria-label="clock emoji" className="text-3xl">
        ⏰
      </span>
      <div
        className={`flex gap-2 ${Object.keys(timeLeft).length > 0 && (timeLeft.d ?? 1) < 1 ? "text-secondary animate-pulse" : "text-white"}`}
      >
    ...
    <Card className="border-primary/50 bg-gradient-to-br from-card to-card/80 shadow-custom shadow-primary/20 animate-pulse-glow">
      <CardHeader>
        <div className="flex justify-between items-center">
          ...
          <Badge
            className={`${isActive ? "bg-primary/20 text-primary border-primary/30" : "bg-muted text-muted-foreground"} text-base`}
          >
      ...
      <CardContent className="space-y-8">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          <div className="p-4 bg-background/50 rounded-lg flex flex-col justify-between">
            ...
            <div>
              <p className="font-code font-bold text-secondary text-4xl">
                {formatBTC(roundInfo.totalPrize)}
              </p>
          ...
          <div className="p-4 bg-background/50 rounded-lg flex flex-col justify-between">
            ...
            <div>
              <p className="font-code font-bold text-white text-3xl">
                ...
                {roundInfo.maxTickets.toString()}
              </p>
            </div>
            <div className="mt-2">
              <Progress
                value={participationRate}
                className="h-2 [&>div]:bg-gradient-to-r from-primary to-secondary"
              />
              <p className="text-xs text-right mt-1 text-muted-foreground">
                {participationRate.toFixed(1)}% lleno
              </p>
            </div>
          </div>
          ...
          <div className="p-4 bg-background/50 rounded-lg flex flex-col justify-between">
            ...
            <div>
              <p className="font-code font-bold text-white text-3xl">
                {roundInfo.maxTickets > 0n
                  ? `1/${roundInfo.maxTickets.toString()}`
                  : "N/A"}
              </p>
