...
export async function fetchUserLotteryStats(
  ...
  console.log(`🔄 Fetching lottery stats for ${userAddress} across ${roundCounter} rounds`)
  ...
  try {
      ...
      if (investmentResult.status === 'fulfilled') {
        ...
        if (investment > 0n) {
          ...
          try {
            ...
            console.warn(`⚠️ Failed to fetch tickets for round ${i + 1}:`, error)
          }
          ...
          if (roundResult.status === 'fulfilled') {
            try {
              ...
              console.warn(`⚠️ Failed to check winner for round ${i + 1}:`, error)
            }
          }
        }
      }
    ...
    console.log(`✅ Fetched lottery stats: ${roundsPlayed} rounds, ${totalTickets} tickets`)
