...
export function useCooperativePool() {
  ...
  return {
    ...
    state:
      createPoolMutation.state !== "idle"
        ? createPoolMutation.state
        : joinPoolMutation.state !== "idle"
          ? joinPoolMutation.state
          : leavePoolMutation.state !== "idle"
            ? leavePoolMutation.state
            : claimYieldMutation.state !== "idle"
              ? claimYieldMutation.state
              : closePoolMutation.state !== "idle"
                ? closePoolMutation.state
                : "idle",
