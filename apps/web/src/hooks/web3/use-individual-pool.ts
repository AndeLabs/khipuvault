...
export function useIndividualPool() {
  ...
  const { data: totalMusdDepositedData, isLoading: loadingTotalMusdDeposited } =
    useQuery({
  ...
  const { data: performanceFeeData, isLoading: loadingPerformanceFee } =
    useQuery({
  ...
  // Format pool stats
  const poolStats = {
    totalMusdDeposited: BigInt(
      (totalMusdDepositedData as unknown as bigint) || 0n,
    ),
  ...
  const performanceFee =
    Number((performanceFeeData as unknown as bigint) || 0n) || 100;

  // Debug tools for troubleshooting (no-op in production)
  const debugTools = {
    manualRefetch: () =>
      queryClient.refetchQueries({ queryKey: ["individual-pool"] }),
    invalidateAll: () =>
      queryClient.invalidateQueries({ queryKey: ["individual-pool"] }),
    getQueryState: () =>
      queryClient.getQueryCache().findAll({ queryKey: ["individual-pool"] }),
    getCurrentData: () => ({
