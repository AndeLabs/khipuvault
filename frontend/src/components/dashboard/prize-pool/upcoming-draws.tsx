...
const upcoming = [
  ...
  { name: "Ronda de Aniversario", date: "Inicia en 12 días", prize: "Est. 1 BTC" },
];
    ...
    <div className="sticky top-24 space-y-6">
        <Card className="bg-card border-primary/20 shadow-custom">
            <CardHeader>
                ...
                <CardDescription>¡Prepárate para las siguientes rondas!</CardDescription>
            </CardHeader>
                ...
                {upcoming.map((item, index) => (
                    <div key={index} className="p-3 bg-background/50 rounded-lg space-y-2">
                        ...
                        <div className="flex justify-between items-end text-sm">
                            <div>
                                <p className="text-muted-foreground text-xs">Premio Estimado</p>
                                <p className="font-bold text-secondary">{item.prize}</p>
                            </div>
                            <Button variant="default" size="sm"><Bell className="mr-2 h-4 w-4"/> Notificarme</Button>
                        </div>
